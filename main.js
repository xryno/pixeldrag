...
let keyProtect = 0
let keyProtectRev = 0
let launchRev = getCurrentRotation(needle);
let launchSpeed = 0
...
//get rotation point of needle

  function getCurrentRotation(el){
  let style = window.getComputedStyle(el);
  let transF = style.getPropertyValue("transform")
  let values = transF.split('(')[1].split(')')[0].split(',');
...
function keyUpPresses(press) {
  ...
  if(press.keyCode === keyProtectRev) {
    ...
    launchRev = getCurrentRotation(needle);
    pitchChange = 0.02
  }
  ...
  //launch quality
        ...
        wheel1.style.setProperty("animation", "wheels 0.08s ease-out infinite");
